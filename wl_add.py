...
def add_date(wl_obj, entry, edit=False):
    ...
    while not valid:
        ...
        wl_obj.help.print_help(wl_obj.show_help, "Task Date", "_ah_date")
        ...
        if edit:
            prompt = (
              "Please enter a new date for this task, or press [ENTER] to " +
...
def add_duration(wl_obj, entry, edit=False):
    ...
    while not valid:
        ...
        # Build prompt.
        if edit:
            prompt = ("Please enter a new duration or end time for this " +
            "task, or press [ENTER] to leave the duration unchanged:")
        else:
            prompt = "Please enter the duration or end time for this task:"
        # Print the prompt and get the response.
        response = io_utils.get_input(prompt)
        ...
        if duration != None:
            ...
            msg = (
              f"{response} could not be interpreted as a valid duration or " +
              "end time.")
            wl_resource.print_status("error", msg=msg)
        # end if
...
def add_note(wl_obj, entry, edit=False):
    ...
    # end if
    # Print the appropriate help text.
    wl_obj.help.print_help(wl_obj.show_help, "Task Notes", "_ah_note")
    # Print navigational instructions.
    if not edit:
        wl_resource.print_nav(q=True, b=True)
    # end if
    ...
    if edit:
        ...
        prompt = ("Please enter a note for this task, or press [ENTER] to " +
        "continue.\n>>  ")
    # end if
    # Print the prompt and get the response.
    response = io_utils.get_input(prompt)
    ...
    if edit and response == "":
        return 0
    # end if
    # The following are only valid if the entry is being created, not
    #  edited:
    if not edit:
        # If the user chose to abort, exit immediately.
        if re.match(r"-q", response, re.I):
...
def add_recurrance(wl_obj, entry):
    """-----------------------------------------------------------------
       ...
       -----------------------------------------------------------------
    """
    # Run in a loop until a valid response is obtained (function exits
    #  immediately then).
    while True:
        # Refresh the entry's attributes.
        _screen_reset(wl_obj, entry, wl_obj.total_entries + 1)
        ...
        # Print navigational instructions.
        wl_resource.print_nav(q=True, b=True)
        # Print the prompt and get the response.  (Don't use the yes_no
        #  function because the user can also toggle help here.)
        ...
        # If the user chose No, set the attribute and return.
            ...
            return 1, []
        # If the user chose Yes, set the attribute and go the function
        #  that determines recurrances.
        elif response[0].lower() == "y":
            entry.recurring = True
            rec_list = _find_recurrances(wl_obj, entry)
            # If recurrances were successfully determined, return the
            #  list of entries.
            if rec_list:
                return 1, rec_list
        ...
        # Otherwise print an error message and try again.
            ...
            wl_resource.print_status(
              "error", msg=f"{response} was not a valid response.")
        # end if
    # end while
# end function
...
def add_time(wl_obj, entry, edit=False):
    ...
    while not valid:
        ...
        # end if
        # Print the appropriate help text.
        wl_obj.help.print_help(wl_obj.show_help, "Task Time", "_ah_time")
        # Print navigational instructions.
        if not edit:
            wl_resource.print_nav(q=True, b=True)
        # end if
...
def add_title(wl_obj, entry, edit=False):
    ...
    while not valid:
        ...
        # end if
        # Print the appropriate help text.
        wl_obj.help.print_help(wl_obj.show_help, "Task Title", "_ah_title")
        # Print navigational instructions.
        if not edit:
        ...
        if edit:
            prompt = (
              "Please enter a new title for this task, or press [ENTER] to " +
        ...
        # If the user chose to toggle help, do that.
          ...
          (wl_obj.show_help == True and response.lower() == "-help off") or
          (wl_obj.show_help == False and response.lower() == "-help on")):
            wl_obj.show_help = not(wl_obj.show_help)
            continue
        # The task title can be any string, as long as it isn't empty.
        elif response != "":
    ...
    while not valid:
        ...
        # Irregular recurrance.
            ...
            day_int = io_utils.menu(
              ...
              "One or more specific days of the month."], keystroke_list="#")
            if day_int == 0:
                confirm = io_utils.yes_no(
                  "Are you sure you want make this task non-recurring?")
                if confirm:
            ...
            # Code for weekday recurrance.
                ...
                prompt = (
                  "Select the day or days that you want this task to recur")
                days = io_utils.menu(
                ...
                prompt = (
                  f"Do you want this task to recur every {day_list} or less" +
                  " often?")
                opt_list = [
                ...
                if skip == 0:
                    ...
                    prompt = (
                      "Enter how often you want the task to recur.  For " +
                      "example, enter [4] if you want the task to recur" +
                      f"every fourth {day_list}.  The maximum interval" +
                      "between recurrances is 30 weeks:")
                    good = False
                    while not good:
            ...
            # Code for monthly recurrance.
                ...
                # 2 == days.
                    ...
                    # First select the weekday(s).
                    prompt = (
                      "Select the day or days that you want this task to " +
                      "recur")
                    days = io_utils.menu(
                      wl_resource.days[1:], keystroke_list="#", multiple=True,
                      prompt=prompt)
                    ...
                    # As always, if the user chose to abort, loop back.
                    if ordinal == 0:
                        continue
                    # end if
                    # Reset the screen.
...
def _get_dates():
    ...
    while not valid:
        ...
        if len(dates) == 0:
            ...
            # end if
        # end if
        # Use a set to create an ordered list of non-duplicate dates.
        date_set = set()
        for date in dates:
            if -31 <= int(date) <= 31:
...
def _screen_reset(wl_obj, entry, entry_number):
    ...
    for attr, value in entry.__dict__.items():
        # Display only those attributes that are editable, and have been
        #  entered.
        if (value != None) and (attr not in ["id", "info", "rec_interval"]):
            print(
              f"{attr.title()}:  {wl_resource.format_string(wl_obj, value)}")
        # end if
    # end for
    # Blank line.
    print()
...
def _set_occurrances(wl_obj, entry):
    ...
    #    beyond the last day of any particular month are ignored.
    #
    # The screen should have been reset just prior to the calling of
    #  this function.  Print the string representation of the recurrance
    #  interval.
    ...
    while not good:
        ...
        while not valid:
            ...
            if not end_date:
                ...
                go = io_utils.yes_no("Do you want to try again?")
                if not go:
                    return []
                else:
                    continue
                ...
                print("The last date must be after the date of the original "
                  "task.")
                go = io_utils.yes_no("Do you want to try again?")
                if not go:
                    return []
        ...
        if entry.rec_interval["unit"] == DAILY:
            ...
            # Now start creating dates, starting from the date of the
            #  first occurrance.
        ...
        # Calculate for weekly options.
            ...
            #  (optionally) a skip value.  The skip value determines the
            #  interval between recurrances.
            interval = 7 * entry.rec_interval["skip"]
            # Identify the weekday of the original task (but note that
            #  the task does not necessarily need to recur on this day).
            cur_date = []
            ...
            for day in entry.rec_interval["days"]:
                if day == start_day:
                    ...
                    cur_date.append(
                      entry.date + datetime.timedelta(days=(day - start_day)))
                else:
                    cur_date.append(
                      entry.date + datetime.timedelta(days=(day - start_day +
                      interval)))
            ...
            # If the end date is before the first recurrance, don't do
            #  anything.
            if not(end_date < cur_date[0]):
                # Loop.
                done = False
                while not done:
                    # Loop through the recurrances.  The first one to be
                    #  after the end date breaks the loop.
                    for d in range(len(cur_date)):
                        if cur_date[d] <= end_date:
                            recurrance_list.append(cur_date[d])
                            cur_date[d] += datetime.timedelta(days=interval)
                        else:
                            done = True
                            break
                        # end if
        ...
        # Calculate for monthly options.
            ...
            #  weekdays.  The latter is more complex because the date(s)
            #  that the specific weekday(s) fall upon must be
            #  recalculated for each month.
            ...
            if entry.rec_interval["dates"]:
                ...
                while not done:
                    ...
                    #  backwards from the end of the month), they need
                    #  to be converted to positive dates for each new
                    #  month.  (The original dates need to be stored and
                    #  restored at the end of the loop, so they can be
                    #  correctly converted for the next month.)
                    orig_dates = list(entry.rec_interval["dates"])
                    for d in range(len(entry.rec_interval["dates"])):
                        if entry.rec_interval["dates"][d] < 0:
                            # Need to offset plus one because -1 represents
                    ...
                    for date in entry.rec_interval["dates"]:
                        # The first recurrance to extend beyond the end
                        #  date breaks the loop.
                        try:
            ...
            # Calculate for specific weekdays during the month.
                ...
                while not done:
                    # Both ordinal and days can have multiple values;
                    #  ordinal is the outside loop.
                    for ordl in entry.rec_interval["ordinal"]:
                        ...
                        for day in entry.rec_interval["days"]:
                            ...
                            #  example, the fifth Friday) fell outside
                            #  the month's range.  Bypass that date.
                            if cur_date:
                                if cur_date > entry.date:
                                    if cur_date <= end_date:
                                        recurrance_list.append(cur_date)
                                    else:
                                        # If past the end date, break
                                        #  all three loops.
                                        brk = True
                                        done = True
                                        break
                        ...
                        # end if
                    # end for
                    # Once all the occurances for the month are set, go
        ...
        if go:
            ...
            action = io_utils.menu(
              ...
              prompt="What would you like to do?", quit=False)
            if action == 2:
                return []
            # end if
        # end if
