...
def browse_entries(wl_obj, entry_list, ndx=0):
    ...
    while True:
        ...
        wl_resource.print_status("status", msg=msg, go=True)
        ...
        if response == "B":
            # If the user wants to go back, return the entry list (as
            ...
            # If the user wants to go to the next entry, increment the
            #  index and loop.
            ndx += 1
            continue
        elif response == "E":
            ...
            if io_utils.confirm("delete this entry"):
                ...
                # If there are no more entries, return the empty list.
                if len(entry_list) == 0:
                    return entry_list
                # end if
                # If the index is past the end of the list, reset it.
...
def browse_list(wl_obj, browse_list, start=0):
    ...
    # For entries, append the title, date and time (as strings).
        ...
        for ndx in range(start, start + 9):
            ...
            # If the time had a leading zero stripped, replace it with a
            #  space.
            if time[1] == ":":
                time = f" {time}"
            # end if
    ...
    return io_utils.menu(
      options, prompt="Select an entry to view:", keystroke_list="#", nav=True,
      prev=prev, nxt=nxt)
# end function
...
def display_entry(wl_obj, entry, edit=False):
    ...
    if entry.recurring:
        print("Recurring:  Yes")
        print(
          wl_resource.format_recurrance_pattern(wl_obj, entry, part=True),
          end=" ")
        print(
          "until",
          wl_resource.format_string(wl_obj, entry.rec_interval["end"]),
          end=".")
    elif entry.rec_child_seq:
        parent = wl_search.lookup_entry_by_id(wl_obj, entry.rec_parent)
        print(
          f"Recurrance {entry.rec_child_seq[0]} of {entry.rec_child_seq[1]}.")
        string = (
          wl_resource.format_recurrance_pattern(wl_obj, parent, part=True) +
          " from " +
          wl_resource.format_string(wl_obj, parent.date, short=True) +
          " until " +
          wl_resource.format_string(wl_obj, parent.rec_interval["end"],
          short=True) + ".")
        io_utils.print_block(string)
        print()
    # end if
    else:
        print("Recurring:  No")
...
def _delete_entry(wl_obj, entry_list, ndx):
    ...
    action = None
    # If the entry is part of a series, ask whether to delete just it, 
    #  or the entire series.
    if entry_list[ndx].rec_child_seq:
        action = io_utils.menu(
          ["Delete this task only.",
          f"Delete all {entry_list[ndx].rec_child_seq[1]} occurances of this " +
          "task."], keystroke_list="#",
    ...
    # If the entry is the parent of a series, ask the same question.
        ...
        response = io_utils.menu(
          ["Delete this task only.",
          f"Delete all {entry_list[ndx].rec_total + 1} occurances of this " +
          "task."], keystroke_list="#",
    ...
    if action == DELETE_ONE:
        ...
        # To delete the entire series, loop through all entries and delete
        #  those in the series, including the original.
        for n in range(len(wl_obj.entries) - 1, -1, -1):
            if (
              (wl_obj.entries[n].rec_parent == del_id) or
              (wl_obj.entries[n].id == del_id)):
                # If the entry is also in the entry list, delete it from the
                #  list.
        ...
        #  first child entry becomes the new parent, and the attributes
        #  of all the other child entries are changed to reflect that.
        if entry_list[ndx].rec_total > 1:
            ...
            for n in range(len(wl_obj.entries)):
                ...
                # end if
            # end for
        # BUT if the first child entry is the ONLY entry in the series
        #  remaining, then it becomes a non-recurring task.
        else:
            # Go through the entries, looking for the one child entry.
            for n in range(len(wl_obj.entries)):
                if wl_obj.entries[n].rec_parent == entry_list[ndx].id:
                    # Change the child entry to a regular non-recurring
                    #  task.
                    wl_obj.entries[n].rec_child_seq = None
                    wl_obj.entries[n].rec_parent = None
...
def _delete_from_series(wl_obj, del_entry):
    """-----------------------------------------------------------------
       ...
       -----------------------------------------------------------------
    """
    # When deleting a single occurrance from a series, the other entries
    #  in the series must be "cleaned up" to correctly represent their
    #  positions.
    # Get the occurance number and the total number of recurrances.
    occ_num, total_occ = del_entry.rec_child_seq
    # Find the parent entry and reduce the number of recurrances by 1.
    wl_search.lookup_entry_by_id(wl_obj, del_entry.rec_parent).rec_total = (
      total_occ - 1)
    # Now go through the entries, finding and editing the recurrance
    #  data for each child entry.  (Cannot use a for-loop directly since
    #  we are editing the entries.)
    for ndx in range(len(wl_obj.entries)):
        if wl_obj.entries[ndx].rec_parent == del_entry.rec_parent:
            # Only change the recurrance number for recurrances after
...
def _edit_entry(wl_obj, edit_entry, ndx, total):
    """-----------------------------------------------------------------
       ...
       -----------------------------------------------------------------
    """
    # This function mainly piggybacks on the add functions to alter the
    #  attributes of the log entry object.  All values are preserved via
    #  the info attribute of a working copy until saved by the user.
    changed = False
    ...
    while True:
        ...
        wl_resource.print_status(
          "status", msg = f"Editing {new_entry.info['ndx']}…", go=True)
        # Display the entry.
        display_entry(wl_obj, new_entry, edit=True)
        # Print instructions.
        wl_obj.help.print_help(wl_obj.show_help, "Editing", "_eh_edit")
        options = ["Title", "Date", "Time", "Duration", "Notes"]
        # User selects the field to edit.
        response = io_utils.menu(
          options, keystroke_list="#", prompt="Please select a field to edit:")
        ...
        if response == QUIT:
            ...
            if (
              ...
              io_utils.yes_no("Do you want to save your changes?")):
                # Recalculate the datetime attribute, in case either the
...
def _get_view_choice(number):
    """-----------------------------------------------------------------
        Asks the user how he/she wants to view matching entries.

        Arguments:
        - number -- the number of matching entries found.

        Returns:  0 if the user quits; 1 if the user wants to view a
         list of entries; 2 if the user wants to browse the entries.
       -----------------------------------------------------------------
    """
    return io_utils.menu(
      ["View list of matching entries", "Browse matching entries"],
      keystroke_list="#", prompt=f"Found {number} matches.")
# end function
...
def _update_entry(wl_obj, entry, resort):
    """-----------------------------------------------------------------
       ...
       -----------------------------------------------------------------
    """
    # If the edited entry is part of a series (either as the parent or a
    #  child), ask whether to apply the edits to the entire series or
    #  only the one entry--but ONLY if the date attribute hasn't been
    #  changed.  If the date attribute HAS been changed, apply all the
    #  edits to just the one entry, even if other attributes have also
    #  been changed.
    if (
      (entry.recurring == True or entry.rec_parent) and
      (entry.date == entry.info["date"])):
        response = io_utils.menu(
          ["Edit this task only", "Edit all tasks in the series"],
          keystroke_list="#", prompt="This task is part of a series.  Do you" +
          " want to apply your changes to all the tasks in this series?")
        # If the user chooses to back out, just return without updating.
        if response == QUIT:
            return
        ...
        edit_series = False
    # end if
    # Find the original entry (and, if applicable, any child entries).
    for ndx in range(len(wl_obj.entries)):
        if (
          ...
          (wl_obj.entries[ndx].id == entry.rec_parent)))):
            ...
            # Recalculate the datetime attribute, in case it's changed.
            wl_obj.entries[ndx].datetime = (
              wl_add.add_datetime(wl_obj.entries[ndx]))
            # If title, date or time changed, need to update sort lists.
            if resort:
